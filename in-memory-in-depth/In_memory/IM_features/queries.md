...
2.  Let's begin with a simple query:  *What is the most expensive order we have received to date?*  There are no indexes or views setup for this.  So the execution plan will be to do a full table scan on the LINEORDER table.  Note the elapsed time.
    ...
    This information is in "IM scan CUs memcompress for query low" is the number of times IMCUs of each mecompress type were touched

    However, "IM scan CUs columns accessed" is 88. This is because we accessed 2 columns in the query. (2 *44 =88)

    If the query had a filter condition (where condition) , then those CUs would need to access these columns and "IM scan CUs columns accessed" will be a multiple of "IM scan CUs memcompress for query low"
...
10. Reconnect to SSB user to clear out session level stats
...
18. Now let's re-run our query and see if there is any difference:
 ````
 <copy>
 set timing on
 SELECT lo_shipmode, SUM(lo_ordtotalprice),
